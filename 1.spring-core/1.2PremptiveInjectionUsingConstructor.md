## Primitive Injection Using Constructor Example

### pom.xml 
```xml
<project xmlns="http://maven.apache.org/POM/4.0.0"
	xmlns:xsi="http://www.w3.org/2001/XMLSchema-instance"
	xsi:schemaLocation="http://maven.apache.org/POM/4.0.0 https://maven.apache.org/xsd/maven-4.0.0.xsd">
	<modelVersion>4.0.0</modelVersion>
	<groupId>com.aplis</groupId>
	<artifactId>1.1PremptiveInjectionUsingSetter</artifactId>
	<version>0.0.1-SNAPSHOT</version>
	<name>1.PremptiveInjection</name>
	<description>it is a simple spring project.</description>
	<dependencies>
		<dependency>
			<groupId>org.springframework</groupId>
			<artifactId>spring-beans</artifactId>
			<version>4.3.3.RELEASE</version>
		</dependency>
		<dependency>
			<groupId>org.springframework</groupId>
			<artifactId>spring-context</artifactId>
			<version>4.3.3.RELEASE</version>
		</dependency>
		<dependency>
			<groupId>org.springframework</groupId>
			<artifactId>spring-core</artifactId>
			<version>4.3.3.RELEASE</version>
		</dependency>
	</dependencies>
</project>
```
---
### application-context.xml

```xml
<?xml version="1.0" encoding="UTF-8"?>
<beans xmlns="http://www.springframework.org/schema/beans"
	xmlns:xsi="http://www.w3.org/2001/XMLSchema-instance"
	xsi:schemaLocation="http://www.springframework.org/schema/beans http://www.springframework.org/schema/beans/spring-beans.xsd">

	<bean id="employee1" class="com.apolis.entity.Employee">
		<property name="ename" value="Timmy" />
		<property name="age" value="21" />
		<property name="salary" value="55000" />
	</bean>
	
	<bean id="employee2" class="com.apolis.entity.Employee">
		<constructor-arg name="ename" value="John" />
		<constructor-arg name="age" value="23" />
		<constructor-arg name="salary" value="22000" />
	</bean>
</beans>

```
---
### School.java

```java
/**
 * This class implements Primitive injection  using Constructor and setter.
 * @author Heeren
 * @version 1.0
 */
package com.apolis.entity;

public class Employee {
	private String ename;
	private int age;
	private int salary;

	public Employee(String ename, int age, int salary) {
		super();
		this.ename = ename;
		this.age = age;
		this.salary = salary;
	}

	public Employee() {
		super();
	}
	
	public String getEname() {
		return ename;
	}

	public void setEname(String ename) {
		this.ename = ename;
	}

	public int getAge() {
		return age;
	}

	public void setAge(int age) {
		this.age = age;
	}

	public int getSalary() {
		return salary;
	}

	public void setSalary(int salary) {
		this.salary = salary;
	}

	@Override
	public String toString() {
		return "Employee [ename=" + ename + ", age=" + age + ", salary=" + salary + "]";
	}

}

	
```
---

### MainClass.java
```java
package com.apolis.main;

import org.springframework.context.ApplicationContext;
import org.springframework.context.support.ClassPathXmlApplicationContext;
import com.apolis.entity.Employee;

public class MainClass {

	public static void main(String[] args) {
		ApplicationContext context = new ClassPathXmlApplicationContext("\\com\\apolis\\common\\application-context.xml");
		Employee employee = (Employee) context.getBean("employee2");
		System.out.println(employee);
	}
}

```
---